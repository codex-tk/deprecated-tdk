...
public:
	...
	tdk::network::address local_address( void ) const;
	tdk::network::address remote_address( void ) const;

	bool connect( const tdk::network::address& address ) const;
	socket accept( tdk::network::address& address ) const;

	int recv( void* buf , int size , const int flag = 0 ) const;
	int send( void* buf , int size , const int flag = 0 ) const;

	int recvfrom(void* buf , int size , tdk::network::address& address , const int flag  = 0 ) const;
	int sendto(  void* buf , int size ,  const tdk::network::address& address , const int flag  = 0 ) const;

	bool wait_for_recv( const tdk::time_span& wait ) const;
	bool wait_for_send( const tdk::time_span& wait ) const;

    bool connect_time_out( const tdk::network::address& address ,  const tdk::time_span& wait );
	...
	bool connect_time_out( const tdk::network::address& address , const int msec );
